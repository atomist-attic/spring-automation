...
const CreateRepoCommandPath = "/command/flask-generator";

export function addFlaskRoutes(express: exp.Express, ...handlers: exp.RequestHandler[]) {

    logger.debug("Adding express routes for flask");

    express.get("/flask/create", ...handlers, (req, res) => {
        const defaults = new NodeGenerator(InMemoryStore);
        return res.render("flask/fillInProject.html", {
            orgs: req.user.orgs,
